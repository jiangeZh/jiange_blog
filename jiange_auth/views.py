...
class UserControl(View):
    ...
    def register(self,request):
        ...
        form = jiangeUserCreationForm(request.POST)
    ...
    def forgetpassword(self,request):
        ...
        form = jiangePasswordRestForm(request.POST)
    ...
    def resetpassword(self,request):
        ...
        try:
            ...
            user = jiangeUser._default_manager.get(pk=uid)
        except (TypeError, ValueError, OverflowError, jiangeUser.DoesNotExist):
            user = None
    ...
    def changetx(self,request):
        ...
        try:
	        #上传头像到七牛
            from jiange_blog.settings import qiniu_access_key,qiniu_secret_key,qiniu_bucket_name
            import qiniu
            ...
            request.user.img = "http://jiange.qiniudn.com/"+filename + "?v" + time.strftime('%Y%m%d%H%M%S')
            request.user.save()
