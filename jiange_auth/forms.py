...
class jiangeUserCreationForm(forms.ModelForm):
    ...
    def clean_username(self):
        ...
        try:
            jiangeUser._default_manager.get(username=username)
        except jiangeUser.DoesNotExist:
            return username
    ...
    def clean_email(self):
        ...
        #判断是这个email 用户是否存在
        try:
            jiangeUser._default_manager.get(email=email)
        except jiangeUser.DoesNotExist:
            return email
        raise forms.ValidationError(
    ...
    def save(self, commit=True):
        user = super(jiangeUserCreationForm, self).save(commit=False)
...
class  jiangePasswordRestForm(forms.Form):
    ...
    def clean(self):
        ...
        if username and email:
            try:
                self.user = jiangeUser.objects.get(username=username,email=email,is_active=True)
            except jiangeUser.DoesNotExist:
                raise forms.ValidationError(
